"use strict";
// =============================================================================
// FORETMOOV — SCRIPT PRINCIPAL v2.0
...
const Onboarding = {
  ...
  finish() {
    ...
    Toast.show("Bienvenue sur ForetMOOV !", "Trouvez vos partenaires de course a Rennes", "green", "leaf");
  }
};
...
const Security = {
  ...
  toggleAccessibility(input) {
    document.documentElement.style.fontSize = input.checked ? "19px" : "16px";
    Store.update("settings.accessibility", input.checked);
    Toast.show(input.checked?"Accessibilite ON":"Accessibilite OFF", "Police ajustee", "blue", "low-vision");
  },
...
const HomeView = {
  async init() {
    ...
    if (heroGreeting) heroGreeting.textContent = `${greeting}, ${Store.data.myProfile.name.split(" ")[0]} \u{1F44B}`;
...
const Boot = {
  async run() {
    ...
    if (Store.data.onboardingDone) {
      setTimeout(() => Toast.show("ForetMOOV", `${LiveCounter._current} sportifs actifs maintenant`, "green", "leaf"), 2800);
    }
  },
};
