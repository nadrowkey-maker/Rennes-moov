...
const UserProfile = {
  async show(user) {
    ...
    document.getElementById("modal-poc-content").innerHTML = `
      ...
      <div style="display:flex;gap:10px;">
        <button class="btn btn-primary btn-lg" style="flex:1;" onclick="PocSystem.send(${user.id})"><i class="fas fa-paper-plane"></i> Envoyer un Poc</button>
        <button class="btn btn-energy btn-md" onclick="ClubSystem.inviteToClub(${user.id})"><i class="fas fa-users"></i> Inviter</button>
      </div>`;
    Modal.open("poc");
  },
};
...
const Boot = {
  async run() {
    ...
    if (Store.data.onboardingDone) {
      setTimeout(() => Toast.show("RennesMOOV", `${LiveCounter._current} sportifs actifs maintenant`, "green", "leaf"), 2800);
    }
  },
};
...
const TrailDetailExtra = {
  ...
  drawMiniMap(trailId){
    ...
    try {
      const map=L.map(id,{zoomControl:false,attributionControl:false,scrollWheelZoom:false,dragging:false});
      ...
      if(wps.length>0){
        ...
        L.polyline(latlngs,{color:'#40916C',weight:4,opacity:0.85,dashArray:'8,4'}).addTo(map);
        wps.forEach((wp,i)=>{
          const icon=L.divIcon({className:'',html:`<div style="width:22px;height:22px;background:${i===0?'#40916C':i===wps.length-1?'#F97316':'white'};color:${i===0||i===wps.length-1?'white':'#40916C'};border:2.5px solid ${i===0?'#40916C':'#F97316'};border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:900;box-shadow:0 2px 6px rgba(0,0,0,0.2);">${i+1}</div>`,iconSize:[22,22],iconAnchor:[11,11]});
